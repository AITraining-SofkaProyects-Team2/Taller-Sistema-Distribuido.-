...
5. Asegurarse de que el test unitario que crees inicialmente falle, ya que estás trabajando en la etapa RED de TDD. Esto ayudará a guiar el desarrollo del código en las etapas posteriores de TDD. No debe fallar por estar mal hecho, sino por no tener la implementación aún.

6. Una vez creado el test unitario, marcarlo en el índice de Tests del documento `TEST_PLAN.md` como "RED" en la celda de completado para indicar que el test está siendo desarrollado. Esto ayudará a mantener un seguimiento claro del progreso de los tests y a facilitar la colaboración con el equipo de desarrollo.
