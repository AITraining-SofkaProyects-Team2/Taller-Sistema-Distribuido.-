...
21. Paginacion: confirmas usar `page + pageSize` o `limit + offset`?
22. Orden por defecto: confirmas `createdAt desc` o prefieres `createdAt asc` o `prioridad`?
23. Filtro de fecha: aplica sobre `createdAt`, `processedAt` o ambos?
24. Zona horaria para filtros de fecha: UTC o hora local?
25. Esquema minimo de BD: confirmas una sola tabla `tickets` con indices por `status`, `priority`, `type`, `createdAt`, `lineNumber`?
26. Herramienta de migraciones: prefieres `prisma`, `typeorm`, `knex` o migracion manual?
